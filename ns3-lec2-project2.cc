...
main (int argc, char *argv[])
{
  ...
  if (nWifi > 250 || nCsma > 250 || nCsma0 > 250)
    {
...
//创建wifista无线终端，AP接入点
  ...
  wifiStaNodes.Add(p2pNodes0.Get (0));
  ...
  for(uint n=0;n<wifiStaNodes.GetN();n++)
  {
        ...
        haha->SetVelocity(Vector(12,0,0));
  }
...
//第二个客户端
  //回显客户端放在最后的STA节点，指向CSMA网络的服务器，上面的节点地址，端口为9
  UdpEchoClientHelper echoClient0 (csmaInterfaces.GetAddress (nCsma), 9);
  ...
  ApplicationContainer clientApps0 = 
    echoClient0.Install (csmaNodes0.Get (nCsma0 - 1));
...
//第三个客户端
  ...
  ApplicationContainer clientApps1 = 
    echoClient1.Install (csmaNodes.Get (nCsma0 - 1));
...
//第四个客户端
  ...
  if (tracing == true)
    {
      pointToPoint.EnablePcap ("ns3-lec2-project2",p2pDevices.Get(0));
      pointToPoint.EnablePcap ("ns3-lec2-project2",p2pDevices.Get(1));
      phy.EnablePcap ("ns3-lec2-project2", apDevices.Get (0));
      csma.EnablePcap ("ns3-lec2-project2", csmaDevices.Get (0), true);
    }
